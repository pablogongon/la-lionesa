...
const AddRating:React.FC<AddRatingProps> = ({product, user}) => {
   ...
   const onSubmit:SubmitHandler<FieldValues> = async(data) =>{
    ...
    if(data.rating === 0) {
        setIsLoading(false)
        return toast.error('Ninguna calificación seleccionada')
    }
   ...
   const deliveredOrder = user?.orders.some(order => order.products.find(item => item.id === product.id) && order.deliveryStatus === 'delivered')
    ...
    return ( <div className="flex flex-col gap-2 max-w-[500px]">
        <Heading title='Valora este producto'/>
        <Rating onChange={(event, newValue) =>{
            setCustomValue('rating', newValue)
        }}/>
        <Input
        id='comment'
        label="Comentar la valoracion"
        disabled = {isLoading}
        register={register}
        errors={errors}
        required
        />
        <Button label={isLoading ? "Cargando" : 'Valorar'} onClick={handleSubmit(onSubmit)}/>
    </div> );
}
